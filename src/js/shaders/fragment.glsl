...
vec2 warp(vec2 pos, vec2 amplitude) {
    ...
    pos.y *= 1.0 + (pos.x*pos.y)*amplitude.y;
    return pos*0.5+0.5;
}
...
void main() {
    ...
    vec2 warpedUV = warp(vUv, vec2(-0.5));
    ...
    vec2 uvCurrent = vec2(warpedUV.x + myprogress * 0.8 + cameraRotation.x, warpedUV.y - myprogress * 0.5 - cameraRotation.y);
    vec2 uvNext = vec2(warpedUV.x - (1. - myprogress) + cameraRotation.x, warpedUV.y + (1. - myprogress)*0.3 - cameraRotation.y);
